...
class Client {
  ...
  constructor(socket) {
    ...
    this.#sendMessage(HI, { version: `${VERSION}` })
  }
  ...
  #handleDataEvent(data) {
    ...
    if (this.#pending.length > MAX_PENDING) {
      ...
      this.#sendMessage(HANGUP, { reason: 7001 })
      // students do not have to detect this, so this is not included in the protocol
      this.#socket.destroy()
    }
  }
   ...
   */
  #processEnter(payload) {
    let username = payload["username"] || ""
    if (this.#username) {
      // cannot enter twice
      this.#sendMessage(LOGON_RESP, { status: "ERROR", code: 5002 })
    } else if (!USERNAME.test(username)) {
      // username must be syntactically valid
      this.#sendMessage(LOGON_RESP, { status: "ERROR", code: 5001 })
    } else if (users[username]) {
      // username already used by other client
      this.#sendMessage(LOGON_RESP, { status: "ERROR", code: 5000 })
    } else {
      ...
      this.#sendMessage(LOGON_RESP, { status: "OK" })
      printStats()
    }
  }
  ...
  #startHeartbeat() {
    ...
    this.#pingTimer = setInterval(() => {
      ...
      this.#pongTimer = setTimeout(() => {
        ...
        this.#sendMessage(HANGUP, { reason: 7000 })
        this.#socket.destroy()
      }, PING_TIMEOUT_MS)
    }, PING_FREQ_MS)
  }
